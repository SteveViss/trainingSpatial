...
# Convertir des points en lignes {background-color="#3687f9"}
...
sf_ca_cities_wgs84$elev <- extract(altCAN, sf_ca_cities_wgs84)[, 2]
sf_ca_cities_wgs84
```
...
# Flux de travail intégrés {background-color="#3687f9"}
...
lines_elev <- city_lines_sf |>
  mutate(mean_elev = extract(altCAN, city_lines_sf, fun = mean, na.rm = TRUE)[, 2])
lines_elev
```
...
#| eval: false
prov_elev <- extract(altCAN, can, fun = mean, na.rm = TRUE)
...
# Cartographie avancée {background-color="#3687f9"}
...
gg <- ggplot() +
  ...
  geom_sf(data = qc, fill = NA, color = "#355c67", linewidth = 0.5) +
  geom_sf(data = sf_ca_cities_wgs84, aes(size = population),
