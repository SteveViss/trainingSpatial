...
- Créer des cartes interactives avec `mapview`
- Créer des cartes statiques avec `ggplot2` et `tmap`

## Installation des packages

```r
install.packages(c(
  "sf", "terra", "tmap", "mapview",
  "geodata", "rnaturalearth", "tidyterra",
  "dplyr", "ggplot2"
))
```
...
![](img/projection1.png){fig-align="center" width="100%"}
:::
::: {.column width="50%"}
**Projeté**

![](img/projection2.png){fig-align="center" width="100%"}
:::
::::
...
Chaque CRS est identifié par un code **EPSG** 
...
On peut trouver des systèmes de coordonnées: 

1. sur [epsg.io](https://epsg.io/)
2. sur [spatialreference.org](https://spatialreference.org/)
3. dans la documentation de `sf` (`?st_crs`)
...
Trait de côtes sur [Natural Earth](https://www.naturalearthdata.com/)

À [télécharger](https://naciscdn.org/naturalearth/10m/physical/ne_10m_coastline.zip) et regarder le contenu du dossier `ne_10m_coastline`
...
Extraire le golfe du Saint-Laurent depuis le *shapefile* du trait de côtes:
...
```{r}
#| label: filter-attributs-dplyr
mb_cities <- sf_ca_cities_wgs84 |> dplyr::filter(admin == "Manitoba")
```
...
mapview::mapview(mb_cities, label = "city")
```
...
mapview::mapview(buf_10K_cities, alpha.regions = 0.3)
```
...
1. Télécharger et lire avec `sf` le [CSV des villes canadiennes](https://raw.githubusercontent.com/SteveViss/trainingSpatial/refs/heads/main/data/ca_cities.csv)
...
*Multibandes / Multicouches (satellite)*
```
...
```{r}
#| label: raster-image
gsl_bbox <- ext(-71.2, -55.1, 45.5, 51.8)
mapview::mapviewOptions(legend = FALSE)
mapview::mapview(crop(ocean_bottom, gsl_bbox))
```
...
mapview::mapview(can, zcol = "mean_elev")
```
...
tm_shape(alt_qc, is.master = TRUE) +
